...
class DuelNetwork:
    def __init__(self,components):
        [W_conv1,b_conv1,W_conv2,b_conv2,W_conv3,b_conv3,W_fc1,b_fc1,W_fc2,b_fc2,W_fc_adv,b_fc_adv] = components
...
class DuelDQN:
    def __init__(self,model_dir='duel_dqn',initStd=0.1,lr=0.00025):
        ...
        # Build model here
        self.model_active = DuelNetwork(self.createNetwork())
        self.model_target = DuelNetwork(self.createNetwork(isTrainable=False))
    ...
    def createNetwork(self,isTrainable=True):
        ...
        W_fc_adv = self.weight_variable([3136,1],isTrainable)
        b_fc_adv = self.weight_variable([1],isTrainable)

        return [W_conv1,b_conv1,W_conv2,b_conv2,W_conv3,b_conv3,W_fc1,b_fc1,W_fc2,b_fc2,W_fc_adv,b_fc_adv]
    ...
    def networkForward(self,model,input):
        ...
        h_adv = tf.matmul(h_fc1,model.W_fc2) + model.b_fc2
        
        h_v= tf.nn.relu(tf.matmul(h_conv3_flat,model.W_fc_adv) + model.b_fc_adv)
